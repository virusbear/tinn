...
object ColorBufferPool: BufferPool<ColorBufferPool.Descriptor, ColorBuffer>({ ColorBufferPool.create(it) }, ColorBuffer::destroy) {
    private fun create(descriptor: Descriptor): ColorBuffer =
        Driver.driver.createColorBuffer(descriptor.size.x, descriptor.size.y, descriptor.contentScale, ColorFormat.RGBA8, MultiSample.None, MipMapLevel.None)

    data class Descriptor(
        val size: IVec2,
        val contentScale: Double = 1.0
    )
}