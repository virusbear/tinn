...
class WindowGL(
...
): ConfinedBindable, Window, Trackable() {
    companion object {
        fun create(width: Int, height: Int, title: String, resizable: Boolean, vsync: Boolean, multisample: MultiSample = MultiSample.None): Window {
            val window = Driver.driver.scheduler.execute {
    ...
    override val open: Boolean
        get() = Driver.driver.scheduler.execute { !glfwWindowShouldClose(native) }

    override var size: IVec2 = getWindowSize()
        private set

    override var contentScale: Double = Driver.driver.scheduler.execute { getWindowContentScale() }
        private set

    override fun clear() {
        Driver.driver.scheduler.execute { GL30C.glClear(GL30C.GL_COLOR_BUFFER_BIT or GL30C.GL_DEPTH_BUFFER_BIT) }
    }
    ...
    override fun destroy() {
        ...
        Driver.driver.scheduler.execute { glfwDestroyWindow(native) }
    ...
    private fun getWindowSize(): IVec2 {
        ...
        Driver.driver.scheduler.execute { glfwGetWindowSize(native, width, height) }
    ...
    private fun getWindowContentScale(): Double {
        ...
        Driver.driver.scheduler.execute { glfwGetWindowContentScale(native, scale, ignored) }
    ...
    private fun getWindowPos(): IVec2 {
        val x = IntArray(1)
        val y = IntArray(1)
        Driver.driver.scheduler.execute { glfwGetWindowPos(native, x, y) }
